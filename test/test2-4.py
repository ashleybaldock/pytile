...
# Test 3, Mk.4 - Adds Wang Tiles for more natural looking grounds
...
class Texture:
    ...
    def __init__(self):
        ...
        if Texture.image is None:
            Texture.image = pygame.image.load("wang_textures.png")
            ...
            a = []
            for x in range(8):
                a.append(pygame.Surface((p,p)))
                a[x].blit(Texture.image, (0,0), ((x * p), (2*p), p,p))
            Texture.textures.append(a)

            # Now load the grass texture
            a = []
            for x in range(8):
                a.append(pygame.Surface((p,p)))
                a[x].blit(Texture.image, (0,0), ((x * p), 0, p,p))
            Texture.textures.append(a)
            ...
            for x in range(8):
                a.append(pygame.Surface((p,p)))
                a[x].blit(Texture.image, (0,0), ((x * p), p, p,p))
            Texture.textures.append(a)
...
class RGBmap:
    ...
    def __init__(self):
        if RGBmap.image is None:
            ...
            RGBmap.image = RGBmap.image.convert()
            RGBmap.maps = []
            # Now load the individual textures into the textures array
            for i in range(0,4):
...
class Lightmap:
    ...
    def __init__(self):
        ...
        if Lightmap.image is None:
            ...
            Lightmap.lightmap_tiles.append(Lightmap.lightmap_tiles[0])
            ...
            for i in range(1,15):
                Lightmap.lightmap_tiles.append(pygame.Surface((p,p)))
                Lightmap.lightmap_tiles[(i + 45)].blit(Lightmap.image, (0,0), ((i * p), p * 4, p,p))
...
class Highlight(pygame.sprite.Sprite):
    ...
    def __init__(self, rect=None, val=0):
        ...
        if Highlight.image is None:
            bb = pygame.image.load("highlight.png")
            ...
            for i in range(4):
                ...
                for j in range(2):
                    c = []
                    for k in range(8):
                        c.append(pygame.Surface((p,p)))
                        c[k].blit(Highlight.image, (0,0), ((k * p), ((i+j) * p), p,p))
##                        c[k].set_colorkey((231,255,255), pygame.RLEACCEL)
                    b.append(c)
                Highlight.tile_images.append(b)

##            Highlight.tile_images.append(pygame.Surface((p,p)))
...
##            Highlight.tile_images[0].set_alpha(130)
        ...
        self.image = Highlight.tile_images[xx][yy][zz]
...
class Tile:
    ...
    def __init__(self):
        ...
        if Tile.image is None:
            ...
            # Now load the ground images, these come in an array thus:
            # Tile.tile_images
            #               |-> [0]: Beach
            #                           |-> [0]: Wang 0
            #                                       |-> [0]: Normal height
            ...
            for k in range(0,1):
                c = []
                for i in range(0,15):
                    c.append(render.MakeTile2(0, i, k, Texture.textures[0][0], Lightmap.lightmap_tiles))
                b.append(c)
            a = []
            for j in range(0,8):
                a.append(b)
            Tile.tile_images.append(a)
            ...
            for j in range(0,8):
                ...
                for k in range(0,1):
                    c = []
                    for i in range(0,15):
                        c.append(render.MakeTile2(0, i, k, Texture.textures[1][j], Lightmap.lightmap_tiles, RGBmap.maps, Texture.textures[0][j]))
                    b.append(c)
                a.append(b)
            Tile.tile_images.append(a)
            ...
            for j in range(0,8):
                ...
                for k in range(0,1):
                    c = []
                    for i in range(0,15):
                        c.append(render.MakeTile2(0, i, k, Texture.textures[1][j], Lightmap.lightmap_tiles))
                    b.append(c)
                a.append(b)
            Tile.tile_images.append(a)
            ...
            for j in range(0,8):
                ...
                for k in range(0,1):
                    c = []
                    for i in range(0,15):
                        c.append(render.MakeTile2(0, i, k, Texture.textures[2][j], Lightmap.lightmap_tiles, RGBmap.maps, Texture.textures[1][j]))
                    b.append(c)
                a.append(b)
            Tile.tile_images.append(a)

            # mountain
            # Now make some mountain tiles (these will come in multiple height varients)
            b = []
            for k in range(0,3):
                c = []
                for i in range(0,15):
                    c.append(render.MakeTile2(0, i, k, Texture.textures[2][0], Lightmap.lightmap_tiles))
                b.append(c)
            a = []
            for j in range(0,8):
                a.append(b)
    ...
    def PaintLand(self):
        ...
        for y in range(self.WorldY):
            for x in range(self.WorldX):
                ...
                pp = 0
                # Find the height range of this tile
                ...
                if self.array[x][y][0] == 0:
                    h = 1
                    th = 0
                if self.array[x][y][0] > 0 and self.array[x][y][0] < 3:
                    h = 2
                    th = 0
                ...
                if self.array[x][y][0] > 2:
                    h = 4
                    th = 0
                if self.array[x][y][0] > 3:
                    h = 4
                    th = 2
                    pp = (self.array[x][y][0] - 4) * ph * 2

                xpos = (self.WidthX / 2) + (x * (p/2)) - (y * (p/2)) - (p/2) + self.dxoff
                ypos = (x * (p/4)) + (y * (p/4)) - ((self.array[x][y][0] * ph) + (pp)) + self.dyoff

                im = Tile.tile_images[h][self.array[x][y][2]][th][self.array[x][y][1]]
            # Now load the ground images, these come in an array thus:
            # Tile.tile_images
            #               |-> [0]: Beach
            #                           |-> [0]: Wang 0
...
##        self.highlight.add(Highlight(pygame.Rect(xpos, ypos, p, p), 0))
        ...
        if self.array[x][y][1] == 0:
            front = 6
            back = 6
        elif self.array[x][y][1] == 1:
            front = 6
            back = 5
        elif self.array[x][y][1] == 2:
            front = 1
            back = 2
        elif self.array[x][y][1] == 3:
            front = 1
            back = 1
        elif self.array[x][y][1] == 4:
            front = 4
            back = 6
        elif self.array[x][y][1] == 5:
            front = 4
            back = 5
        elif self.array[x][y][1] == 6:
            front = 2
            back = 2
        elif self.array[x][y][1] == 7:
            front = 2
            back = 1
        elif self.array[x][y][1] == 8:
            front = 0
            back = 3
        elif self.array[x][y][1] == 9:
            front = 0
            back = 0
        elif self.array[x][y][1] == 10:
            front = 5
            back = 4
        elif self.array[x][y][1] == 11:
            front = 5
            back = 7
        elif self.array[x][y][1] == 12:
            front = 3
            back = 3
        elif self.array[x][y][1] == 13:
            front = 3
            back = 0
        elif self.array[x][y][1] == 14:
            front = 7
            back = 4
        # Sprite for the back part of the highlight
        val = (0, 0, front)
        self.highlight.add(Highlight(pygame.Rect(xpos, ypos, p, p), val))
        
        # Sprite for the front part of the highlight
        val = (0, 1, back)
        self.highlight.add(Highlight(pygame.Rect(xpos, ypos, p, p), val))
